...
export const handler: Handler = async (event) => {
    ...
    try {
        ...
        const { data: sigRequest, error } = await supabase
            ...
            .select('id, status, signer_name, token_expires_at, device_id')
            .eq('token', token)
            .single()
        if (error || !sigRequest) {
        ...
        // Firmato: la posizione non serve piu'. Scaduto: si respinge.
        if (sigRequest.status === 'signed') return { statusCode: 200, body: JSON.stringify({ ok: true, ignorata: true }) }
        if (sigRequest.status === 'expired' || new Date(sigRequest.token_expires_at) < new Date()) {
        ...
        if ((recenti || 0) >= MAX_EVENTI_POSIZIONE_10_MIN) {
            return { statusCode: 429, body: JSON.stringify({ error: 'Troppe richieste' }) }
        }
        ...
        if (lat === null || lon === null || Math.abs(lat) > 90 || Math.abs(lon) > 180) {
            return { statusCode: 400, body: JSON.stringify({ error: 'Coordinate non valide' }) }
        }
